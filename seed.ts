...
import { PostLike } from './src/app/modules/post/postLike.model';
import { Comment } from './src/app/modules/comment/comment.model';

const SALT_ROUNDS = 12;

// Realistic avatar URLs (using UI Faces API & randomuser.me style)
const avatars = [
...
const postContents = [
  ...
  "Working from a cozy café today. Change of scenery really helps with creativity. ☕ Where's your favorite place to work from?",
  "Just completed a 10k run! Physical fitness is just as important as mental fitness. 🏃‍♂️ #Fitness #Health",
...
async function seed() {
  try {
    ...
    for (let i = 0; i < postContents.length; i++) {
      ...
      const hasImage = i < 5; // First 5 posts have images
      ...
      console.log(`  ✅ Post ${i + 1} by ${createdUsers[authorIndex].firstName}: "${postContents[i].substring(0, 50)}..."`);
    }

    // Create likes (random likes between users)
    console.log('\n❤️ Creating likes...');
    let likeCount = 0;
    ...
    for (let i = 0; i < createdPosts.length; i++) {
      ...
      for (let j = 0; j < numComments; j++) {
        // Pick a random user who is NOT the post author
